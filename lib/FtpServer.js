...
function FtpServer(host, options) {
  ...
  self.server.on('connection', self._onConnection.bind(self));
  ...
  self.tlsServer = null;
  if (self.options.tlsPort) {
    self.tlsServer = net.createServer();
    self.tlsServer.on('connection', self._onConnection.bind(self));
    self.tlsServer.on('error', function(err) {
      self.emit('error', err);
    });
    self.tlsServer.on('close', function() {
      self.emit('close');
    });
  }
}
...
FtpServer.prototype.listen = function(port, host, cb) {
  this.server.listen(port, host, (e) => {
    ...
    if (this.tlsServer) {
      this.tlsServer.listen(this.options.tlsPort, this.host, cb);
      return;
    }
    if (cb) cb();
  });
...
FtpServer.prototype.close = function(cb) {
  this.server.close((e) => {
    if (e) {
      if (cb) cb(e);
      return;
